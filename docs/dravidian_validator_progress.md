...
**Final headline:** `entry_match_rate` **69.9% → 98.6%**; `entries_matched` **19,100** of **19,371**
in-scope Starling entries carrying a DED number. Residual **271** rows: **136 `Language only`**,
**135 `No`** — both now measured directly from the row-level results, not inferred. Source of truth:
`data/dravidian/cross-validating-dded-starling/tree_validation_output/tree_validation_summary.json`,
regenerated by a full validator re-run on **2026-09-05**.

> **Why these differ from the 2026-08-21 figures (19,083 / 19,354).** That summary was computed
> against the scrape as it stood on 08-21. `starling_complete_data.json` was then rewritten on
> 08-22 by the scraper colon-artifact fix (`08e14ae`), which carries **17 more entries** and one
> more subgroup-DB orphan. Numerator and denominator both rose by 17, so the rate is unchanged at
> **98.6%** and the residual is **identical** — 136 + 135. Nothing regressed; corpus, parser and
> matcher are untouched. §10's progression table keeps its 08-21 values, since those are the
> measurements each fix was verified against.

> ### Read this number with its three caveats
>
> **1. The denominator is scoped.** 19,371 is the *in-scope* population. The validator excludes
> **654** orphan rows from non-DED-keyed subgroup databases (`ad5da93`; gndet 636, kuiet 5, telet 4,
> ndret 4, braet 3, kogaet 1, gonet 1) that carry no DED number to match on. Against the full
> **20,025**-row scrape the same 19,100 matches are **95.4%**. Never cite 98.6% without stating the
> exclusion.
>
...
> are deliberately *not* counted in the 19,100, and must not be folded into a published rate.
> DED 1818 Muria `gumiya` shows the triage is fallible: it was classified `distinct_reflex` and was
> later matched by `c5f78ff`.
...
`data/dravidian/burrow_ded/review_ledger.json` (force-tracked in git; see `docs/SYNC.md` Tier 1).
...
| `distinct_reflex` | 39 | **no** — Kota `gud-` vs `kur-` |
| `close_spelling_vowel_consonant` | 24 | yes |
| `morphological_citation` | 11 | yes — Gondi `-ānā` / Kuwi `-ali` vs bare stem |
| `display_fallback_or_postposed` | 7 | n/a — matcher showed an English gloss as the "Burrow form" |
| `inscr_telugu_r_d_corresp` | 3 | yes — Inscriptional Telugu `ṛ`/`ḍ` |

Worksheet: `genuine_divergence_worksheet.csv`, **regenerated 2026-09-05** from the fresh row-level
results; it now holds exactly the live 136 rows, and the counts above are the regenerated ones.

The pre-regeneration worksheet listed 144 rows. Joining it against the fresh results gives **8
resolved, 136 still `Language only`, 0 new uncategorized** — every live divergence carries a
category and nothing is unclassified. The 8 that `c5f78ff` recovered did **not** all come from
`display_fallback_or_postposed`, as that commit's DED list might suggest; they crossed four
categories:

| Category | Resolved | DEDs |
|---|---|---|
| `display_fallback_or_postposed` | 4 | 2675 Maria, 1655 Mandla (Phailbus), 1416 Muria + Maria |
| `morphological_citation` | 2 | 3808 Betul, 3399 Mandla (Williamson) |
| `close_spelling_vowel_consonant` | 1 | 3884 Gommu |
| `distinct_reflex` | 1 | 1818 Muria |

The last row matters: DED 1818 Muria `gumiya` had been classified a **distinct reflex** — a
genuinely different word — and the postposed-marker lever then matched it. One mislabel in 144 is a
low rate, but it is direct evidence that the AI triage pass is fallible, and it is the strongest
argument for the human spot-check in §11.
...
entries_matched          19,100   98.6%
  ...
  No                        135
unique_branches           7,953   fully_attested 7,745 · not_attested 44 · ded_not_in_corpus 10
```

Verified by full re-run, 2026-09-05. The `Match` column breaks down as 19,100 `Yes` across nine
match paths (`exact` 14,320 · `gloss_dialect_exact` 2,117 · `substring` 1,080 · `gloss_secondary`
799 · `gloss_dialect_substring` 784), 136 `Language only` (82 at conf 0.95, 54 at 1.00), 135 `No`.

**How to cite this in the AsiaLex paper.** The defensible sentence is: *"98.6% of the 19,371
DED-bearing Starling entries in scope matched a Burrow & Emeneau attestation; 654 orphan rows from
...
1. **Human spot-check of the `genuine_divergence` set — OPEN.** 135 of the ledger's 147
   language-level records are `reviewed_by: claude-triage`. A stratified sample (~20 rows across
   the 6 categories) confirmed by a Dravidianist would convert the residual from asserted to
   evidenced. **DED 1818 shows this is not theoretical**: it was labelled `distinct_reflex` and
   then matched by a later lever, so at least one classification in 144 was wrong. Until this is
   done, describe the residual as *classified*, never as *verified*.
2. **Scrape CVOTGD — OPEN.** All 662 CVOTGD source rows carry a `page_number` and an empty
   `original_entry`. The one remaining data gap in the import payload.
3. **Finish `loop_worklist.json`** — 33 of 52 language groups still `pending`, covering 170 of the
   585 diagnosed rows. `loop_findings.md` carries a dominant verdict and a proposed fix per worked
   group, so this is resumable.

**Done 2026-09-05:** ~~regenerate the worksheet and the row-level artifacts~~ — a full validator
re-run refreshed `tree_validation_results.{csv,xlsx}`, `validation_audit_report.xlsx` and
`coverage_by_ded_paragraph.xlsx` (all had been stuck at their 2026-07-07 build), and the worksheet
was regenerated to the live 136 rows. `triage_queue.csv` is still 2026-06-23; it comes from
`triage_mismatches.py`, not the validator, and only needs rebuilding when triage resumes.
